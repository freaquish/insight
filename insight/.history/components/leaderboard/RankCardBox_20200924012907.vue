<template>
  <div class="w-full h-auto px-2 py-2 flex justify-between bg-white">
    <div class="w-auto flex">
      ...
      <p class="font-muli ml-2 mt-1 font-bold">{{ scorecard.account.name }}</p>
    </div>
    <p class="font-muli font-normal mt-1 text-green-400">
      {{ san_score }}
    </p>
    <p class="font-muli text-pink-600 mt-1 mr-2">
      <span class="material-icons mt-1 stroke-current text-pink-600"
        >emoji_events</span
      >
      <span>{{ san_rank }}</span>
    </p>
