<template>
  <div class="w-full px-4 py-2 rounded-md shadow-lg flex my-1 h-16">
    <div class="inline-block">
      <span v-if="this.progress.type === 'image'">
        <img class="w-8 h-8 mt-2 rounded-sm" :src="progress.url" />
      </span>
    ...
    <div class="w-full h-full flex flex-row-reverse">
      ...
      <div class="w-full h-1 bg-gray-300 mx-3 mt-5 rounded-full">
        ...
        <div v-else class="h-1 w-full bg-purple-600 rounded-full" :style="`width: ${progress.progress}%;`"></div>
      </div>
    </div>
  </div>
</template>
