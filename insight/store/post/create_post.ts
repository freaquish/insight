...
export const actions: ActionTree<RootState, RootState> = {
        ...
        if (state.post != undefined) {
            if (window.navigator.geolocation) {
                window.navigator.geolocation.getCurrentPosition(position => {
                    if (state.post != undefined) {
                        state.post.addCoords({ lat: position.coords.latitude, long: position.coords.longitude })
                    }
                })
            }
            ...
            if (this.$axios.defaults.headers.common['Authorization'] === undefined) {
                ...
                this.$axios.post(url, JSON.stringify(state.post.renderToData())).then(res => {
                    if (res.status === 201) {
                        commit('reset')
                        this.$router.push('/')
                    } else {
    ...
    createPost({ state, dispatch }) {
        if (state.post != undefined && JSON.stringify(state.post.getAssets()) != '{}') {
