...
export const actions = {
  fetchFeed: async function({ state, commit }, index) {
    ...
    let url = 'feed'
    if (
      state.links.next != null ||
      (state.links.next != undefined && state.posts.length > 0)
    ) {
    ...
    this.$axios
      ...
      .then(res => {
        storage.set(
          'avatar',
          res.data.meta.avatar != undefined && res.data.meta.avatar.length > 0
            ? res.data.meta.avatar
            : avatarDefault
        )
